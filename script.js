...
//adicionei função e caminho para som
let somtiro;


function carregarSom(){
    somtiro = new Audio('/assets/audio/tiroJogador.wav');
    somtiro.loop = false;
};
...
document.addEventListener("keydown", (e) => {
    ...
    if (e.key === " " && Date.now() - ultimoTiro > tempoEntreTiros) {
        ultimoTiro = Date.now();
        tiros.push({
            x: navePlayer.x + navePlayer.width / 2 - 1,
            y: navePlayer.y,
            width: 2,
            height: 20,
            dy: -5,
        ...
        //adicionei o som pra tocar
        if (somtiro){
            somtiro.currentTime = 0;
            somtiro.play();
        }
    
        //colocar ao apertar espaço o som para tocar
        if (e.key === " ") {
...
document.addEventListener("keyup", (e) => {
    ...
    //colocar para ao deixar de apertar espaço, som  parar
    if (e.key === " ") {
        ...
        for (let linha = 0; linha < numLinhas; linha++) {
            for (let coluna = 0; coluna < numColunas; coluna++) {
                ...
                if (!existente) {
                    ...
                    let tipoInimigo = Math.floor(Math.random() * 3) + 1;
...
function desenharJogo() {
    ...
    tiros.forEach((tiro, index) => {
        ...
        if (tiro.y < 0 || tiro.tempoVida > 100) {
